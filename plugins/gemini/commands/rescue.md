...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model|pro|flash>] [--dirs <path,...>] [--files <glob,...>] [--max-files <n>] [--max-file-bytes <n>] [--print-command] [what Gemini should investigate, solve, or continue]"
...
- Otherwise, before starting Gemini, check for a resumable rescue transcript from this Claude session by running:
...
- `--print-command` prints the Gemini CLI invocation preview, prompt delivery mode, assembled prompt size, included-file inventory, skipped-file summary, and prompt preview without creating a job or invoking Gemini.
...
- If the helper reports that the Gemini CLI is missing or unauthenticated, stop and tell the user to run `/gemini:setup`.
