...
/**
 * Writes a fake `grok` shim binary into a temp dir and returns the directory.
 ...
 * prompt. This keeps tests hermetic and avoids needing the real Grok CLI.
 *
 ...
 *   --version                          → "grok-fake 0.0.1"
 *   anything else                      → stdout = JSON-encoded canned review
 */
export function writeFakeGrokBinary({ name = "grok" } = {}) {
...
if (process.env.FAKE_GROK_INVOCATION_LOG) {
  fs.appendFileSync(process.env.FAKE_GROK_INVOCATION_LOG, JSON.stringify(process.argv) + "\\n", "utf8");
}

if (args.includes("--version")) {
  process.stdout.write("grok-fake 0.0.1\\n");
  process.exit(0);
}
...
/**
 * Returns env additions that, when merged with process.env, point the grok
 * adapter at the fake shim and supply a fake API key.
 */
export function fakeGrokEnv(fakeBinDir) {
  return {
