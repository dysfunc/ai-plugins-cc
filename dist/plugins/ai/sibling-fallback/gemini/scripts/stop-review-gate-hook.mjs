...
function runStopReview(cwd, input = {}) {
  const scriptPath = path6.join(SCRIPT_DIR, "gemini-companion.mjs");
  ...
  if (result.status !== 0) {
    ...
    return {
      ok: false,
      reason: detail ? `The stop-time Gemini review task failed: ${detail}` : "The stop-time Gemini review task failed. Run /gemini:review --wait manually or bypass the gate."
    };
  }
  try {
...
function main() {
  ...
  const runningTaskNote = runningJob ? `Gemini task ${runningJob.id} is still running. Check /gemini:status and use /gemini:cancel ${runningJob.id} if you want to stop it before ending the session.` : null;
