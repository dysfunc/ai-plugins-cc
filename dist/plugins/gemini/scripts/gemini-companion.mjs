...
var DEFAULT_BIN = process3.env.GEMINI_BIN || "gemini";
var TRANSCRIPT_TURN_CAP = readNumberEnv("GEMINI_PLUGIN_TRANSCRIPT_TURN_CAP", 8);
var TRANSCRIPT_CHAR_CAP = readNumberEnv("GEMINI_PLUGIN_TRANSCRIPT_CHAR_CAP", 4e4);
var ARGV_PROMPT_LIMIT = readNumberEnv("GEMINI_PLUGIN_ARGV_PROMPT_LIMIT", 1e5);
...
async function getGeminiAuthStatus(cwd, options = {}) {
...
async function runGeminiTurn(cwd, options = {}) {
  const availability = getGeminiAvailability(cwd);
  if (!availability.available) {
...
function prepareGeminiTurnInvocation(cwd, options = {}) {
  const prompt = options.prompt?.trim() || options.defaultPrompt || "";
  if (!prompt) {
    throw new Error("A prompt is required for this Gemini run.");
  }
...
async function runGeminiReview(cwd, options = {}) {
...
function spawnDetachedTaskWorker(cwd, jobId) {
  const scriptPath = path7.join(ROOT_DIR, "scripts", "gemini-companion.mjs");
